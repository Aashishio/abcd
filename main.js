function setup() {
  canvas = createCanvas(325, 250);
  canvas.center();
  video = createCapture(VIDEO);
  video.hide();
  classifier = ml5.imageClassifier('MobilNet', modelLoaded);
}

function modelLoaded(){
  console.log("Model Loaded");
}

function draw() {
  image(video,0,0,325,250);
  classifier.classify(video,gotResults);
}

var prevResult = '';

function gotResults(error,results){
  if(error){
    console.error(error);
  }
  else {
    if(results[0].confidence > 0.5 && results[0].label!=prevResult){
      console.log(results);

      prevResult = results[0].label;

      var synth = window.speechSynthesis;
      var speak = 'Object Detected is'+ results[0].label;
      var utterThis = new SpeechSynthesisUtterance(speak);
      synth.speak(utterThis);

      document.getElementById("a").innerHTML = results[0].label;
      document.getElementById("b").innerHTML = results[0].results;
    }
  }
}